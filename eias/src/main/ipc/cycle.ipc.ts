...
export function registerCycleHandlers() {
  ...
  ipcMain.handle("cycle:createCycle", async (_, data) => {
    db.run("INSERT INTO exam_cycles(name,academic_year,status) VALUES(?,?,?)", [data.name, data.academic_year, "draft"])
    return db.queryOne("SELECT * FROM exam_cycles WHERE rowid=last_insert_rowid()")
  })
